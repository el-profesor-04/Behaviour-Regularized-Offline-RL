...
class Agent(object):
  ...
  def __init__(
    ...
    self._aux_train_data = aux_train_data
  ...
  def _build_agent(self):
    ...
    aux_batch = self._get_aux_batch()
    self._feat_ext = None
    self._build_feat_ext()
    self._init_vars(train_batch, aux_batch)
  ...
  def _build_feat_ext(self):      # feat ext for agent daac
  ...
  @property
  def feat_ext(self):            # feat ext for agent daac
    return self._feat_ext
  ...
  def _get_aux_batch(self):
    """Samples and constructs auxiliary batch of transitions."""
    if self._aux_train_data==None:
      return None
    batch_indices = np.random.choice(self._aux_train_data.size, self._batch_size)
    batch_ = self._aux_train_data.get_batch(batch_indices)
    transition_batch = batch_
    batch = dict(
        s1=transition_batch.s1,
        s2=transition_batch.s2,
  ...
  def _optimize_step(self, batch, aux_batch=None):
  ...
  def _init_vars(self, batch, aux_batch=None):
    pass
...
class Config(object):
  ...
  def _get_agent_args(self):
    ...
    agent_args = utils.Flags(
        ...
        aux_train_data=agent_flags.aux_train_data,
        )
