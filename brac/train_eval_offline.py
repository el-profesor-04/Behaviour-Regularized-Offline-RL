...
def train_eval_offline(
  ...
  with tf.device('/cpu:0'):
    ...
    aux_data = dataset.Dataset(observation_spec, action_spec, data_size)
  data_ckpt = tf.train.Checkpoint(data=full_data)
  data_ckpt.restore(data_file)

  if aux_file!=None:
    aux_ckpt = tf.train.Checkpoint(data=aux_data)
    aux_ckpt.restore(aux_file)
  ...
  if aux_file!=None:
    aux_train_data = aux_data.create_view(train_indices)
  else:
    aux_train_data = None

  #print('in train eval offline, aux train_data', aux_train_data,'???')
  ...
  while step < total_train_steps:
    ...
    if step % eval_freq == 0 or step == total_train_steps:
      ...
      eval_result, eval_infos = train_eval_utils.eval_policies(
          tf_env, agent.test_policies, agent.feat_ext, n_eval_episodes)
