...
def eval_policy_episodes(env, policy, feat_ext, n_episodes):
  ...
  for _ in range(n_episodes):
    ...
    while not time_step.is_last().numpy()[0]:
      ...
      if feat_ext!=None:                          # feat ext !None if agent daac
        state = feat_ext(time_step.observation)
...
def eval_policies(env, policies, feat_ext ,n_episodes):  # feat ext !None if agent daac
  results_episode_return = []
  infos = collections.OrderedDict()
  for name, policy in policies.items():
    mean, _ = eval_policy_episodes(env, policy, feat_ext, n_episodes)
